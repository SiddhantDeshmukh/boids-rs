...
fn reset(rng: &mut ThreadRng) -> (Params, Vec<Boid>) {
    ...
    // Create boids
    let num_boids = 200;
    let boids_pop: Vec<Boid> = (0..num_boids)
        .map(|_| random_boid(rng, &bounds))
        .collect::<Vec<Boid>>();
...
/* TODO:
 * - Add time-step controls (sometimes the boids are way too fast)
 * - Add predators that chase the prey boids
 */
